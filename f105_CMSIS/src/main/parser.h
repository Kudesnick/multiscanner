...
// Тип автомата для парсинга
typedef const struct fsm_steps_t
{
    const char * const name;                              // Сигнатура команды
    const bool(* const func)(char * str, uint32_t param); // Функция обработчик
    const uint32_t param                                  // Дополнительный параметр
    const fsm_steps_t * const child;                      // Дочерний автомат
} parser_fsm_steps_t;
...
bool     parser_parse(char * str, uint32_t param); // Собственно, парсим строку
